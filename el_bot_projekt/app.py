...
os.environ["GOOGLE_API_KEY"] = "HÄR KLISTRAR DU IN DIN API-NYCKEL"

# --- 3. ANPASSAD STYLING (CSS) ---
# Denna kod styr utseendet så det matchar bilden.
st.markdown("""
<style>
...
logo_path = os.path.join(current_dir, "bilder", "logo.png") # OBS! Se till att loggan heter logo.png och ligger i bilder-mappen.

# Skapar headern med HTML och CSS
if os.path.exists(logo_path):
    ...
    # Om loggan saknas, visa bara texten
    st.markdown("""
        <div class="brand-header">
            <h1>ISOLERAB El-Assistent</h1>
        </div>
...
def render_content(text):
    """Ritar ut text och letar efter bild-taggar."""
    ...
    for i, part in enumerate(parts):
        if i % 2 == 0:
            if part.strip(): 
                # Vi använder en HTML-span med klassen 'highlight' för att limegröna text-highlights ska fungera
                highlighted_text = part.strip().replace("HIGHLIGHT:", "<span class='highlight'>").replace(":HIGHLIGHT", "</span>")
                st.markdown(highlighted_text, unsafe_allow_custom_html=True)
        else:
